...
export default function DepartmentPage() {
  ...
  const usersDblist = useSelector(selectSubjectTableData);
  const totalRows = useSelector(selectSubjectTableTotalCount);
  ...
  const verticalData = useSelector(selectgetSubjectData);
  // dynamic filgter part
  const currentRole = "SuperAdmin";
  const currentPage = "SubjectPage";
  const verticalType = "Subject";
  ...
  const fetchVerticals = useCallback(
    (page = 0, limit = pageSize) => {
      ...
      dispatch(fetchSubjectList({ token, query, type: verticalType }));
    },
  ...
  const handleReset = useCallback(() => {
    ...
    dispatch(resetSubjectTableState());
    setmappedTeachers([]);
  ...
  const handleEdit = (id) => {
    dispatch(getSubject({ token, id, type: verticalType }));
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flexGrow: 1, overflow: "auto" }}>
        {mappedTeachers.length === 0 ? (
          <Box sx={{ textAlign: "center", p: 2 }}>
            <NoDataFound message="No Subject record found" />
          </Box>
        ) : (
          <VerticalTable
            rows={mappedTeachers}
      ...
      <ConfirmationPopup
        ...
        msg={"Are you sure you want to make this Subject Active ?. "}
